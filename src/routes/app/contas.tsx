...
function Contas() {
  ...
  const { accounts, addAccount, removeAccount, transactions } = useData();
...
function NewAccountDialog({ onSave }: { onSave: (a: { name: string; type: 'corrente' | 'poupanca' | 'dinheiro' | 'investimento'; balance: number }) => void }) {
  const [name, setName] = useState('');
  const [type, setType] = useState<'corrente' | 'poupanca' | 'dinheiro' | 'investimento'>('corrente');
